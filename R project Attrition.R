...
(42/1470)*100
#since yearssincelastpromotion has more than 2% outlier i.e. 2.85% we need to impute, 
#for rest variables we can remove
...
x1 <- out_std_fix(data$TotalWorkingYears)
x1
data <- data[-x1$num,]
skewness(data$TotalWorkingYears)
boxplot(data$TotalWorkingYears)

x2 <- out_std_fix(data$YearsAtCompany)
x2
data <- data[-x2$num,]
skewness(data$YearsAtCompany)
boxplot(data$YearsAtCompany)

x3 <- out_std_fix(data$YearsInCurrentRole)
x3
data <- data[-x3$num,]
skewness(data$YearsInCurrentRole)
boxplot(data$YearsInCurrentRole)

x4 <- out_std_fix(data$YearsSinceLastPromotion)
x4
data <- data[-x4$num,]
skewness(data$YearsSinceLastPromotion)
boxplot(data$YearsSinceLastPromotion)
#ggplot(data,aes(x=data$YearsSinceLastPromotion,y=..density..))+geom_histogram()+geom_density()

x5 <- out_std_fix(data$YearsWithCurrManager)
x5
data <- data[-x5$num,]
...
#dealing with skewness which is not due to outlier

data$YearsAtCompany <- (data$YearsAtCompany)^1/2
skewness(data$YearsAtCompany)
data$YearsSinceLastPromotion <- (data$YearsSinceLastPromotion)^1/3
skewness(data$YearsSinceLastPromotion)
